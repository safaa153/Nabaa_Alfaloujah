// accountent/js/areas/customers_model.js
...
export const CustomersModal = {
    ...
    open: async function(areaId, areaName) {
        ...
        this.title.innerText = `الزبائن في منطقة: ${areaName}`;
        this.body.innerHTML = '<tr><td colspan="4" class="text-center py-4"><i class="ph ph-spinner animate-spin text-2xl text-blue-500"></i></td></tr>';
        this.emptyState.classList.add('hidden');

        try {
            const customers = await AreasData.getCustomersByAreaId(areaId);
            this.renderList(customers);
        } catch (error) {
            console.error("Error fetching customers:", error);
            this.body.innerHTML = '<tr><td colspan="4" class="text-center py-4 text-red-500">حدث خطأ في جلب البيانات</td></tr>';
        }
    },
