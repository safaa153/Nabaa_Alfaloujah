...
export const CustomersModal = {
    ...
    open: async function(driverId, carName) {
        ...
        this.title.innerText = `الزبائن المرتبطين بـ ${carName}`;
        ...
        if (!driverId) {
            ...
            this.emptyState.querySelector('p').innerText = "لم يتم تحديد سائق لهذه السيارة";
            return;
        }

        try {
            const customers = await CarsData.getCustomersByDriverId(driverId);
